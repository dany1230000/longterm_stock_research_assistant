...
class StaticPagesPipelineTests(unittest.TestCase):
    def test_github_pages_full_catalog_import_is_schedule_or_manual_only(self) -> None:
        ...
        self.assertIn("Restore public 00631L price history", workflow)
        self.assertIn("restore_public_00631l_price_history.py", workflow)
        self.assertIn("Restore public ETF price-history rows", workflow)
        self.assertIn("restore_public_etf_price_history.py", workflow)
        ...
        self.assertIn("batch ${batch}/3", workflow)
    ...
    def test_local_pages_build_full_catalog_import_is_explicit(self) -> None:
        ...
        self.assertIn("--restore-public-history", script)
        self.assertIn("--skip-restore-public-history", script)
        self.assertIn("--restore-public-attempts", script)
        self.assertIn("--skip-restore-public-attempts", script)
        ...
        self.assertIn("RESTORE_PUBLIC_HISTORY", script)
        self.assertIn('set "RESTORE_PUBLIC_ATTEMPTS=1"', script)
        self.assertIn('set "RESTORE_PUBLIC_HISTORY=1"', script)
        ...
        self.assertIn("Skipping public 00631L and ETF history restore for this local Pages build", script)
        self.assertIn("Skipping public ETF attempt restore for this local Pages build", script)
        self.assertIn("Remove --skip-restore-public-history", script)
        self.assertIn("Remove --skip-restore-public-attempts", script)
        ...
        self.assertIn("scripts\\00631l_restore_public_price_history.cmd", script)
        self.assertIn("scripts\\00631l_restore_public_etf_price_history.cmd", script)
        self.assertIn("scripts\\00631l_restore_public_etf_attempts.cmd", script)
        self.assertIn("for /L %%I in (1,1,3)", script)
        self.assertIn("batch %%I/3", script)
